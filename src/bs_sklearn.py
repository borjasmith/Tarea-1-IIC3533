...
def fit_single_bootstrap_sklearn(X, y, seed_b):
    ...
    # Fit the model and return coefficients
    model = LinearRegression(fit_intercept=False)
    model.fit(X_b, y_b)
    
    return model.coef_

def bootstrap_sklearn(X, y, B, p, base_seed=1234):
    # Generate B unique seeds to ensure independent samples across processes
    seeds = [base_seed + i for i in range(B)]
    
    start_time = time.time()
    
    # Manually distribute the B iterations across p processes
    coefs = Parallel(n_jobs=p)(
        delayed(fit_single_bootstrap_sklearn)(X, y, seeds[i]) 
        for i in range(B)
    )
...
if __name__ == "__main__":
    ...
    lb, ub, t_exec = bootstrap_sklearn(X, y, B=48, p=1)
